...
if (isset($_GET['idchambre'])) {
 ...
 $stmt = $pdo->prepare('SELECT * FROM chambre WHERE idchambre = ?');
   $stmt->execute([$_GET['idchambre']]);
  $chambre = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$chambre) {
       exit('chambre doesn\'t exist with that id!');
    }
} 
else {
...
<body class="theme-red">
    ...
    <section class="content">
        ...
        <tr>
            <td>date</td>
            <td><input type='text' name='date2'value ="<?php echo $chambre['date2'];?>" class='form-control' /></td>
        </tr>

        
     
                                        <button class="btn btn-success waves-effect" type="submit">Valider</button>
                                        <a href='Ajouterchambre.php' class='btn btn-danger'>Back</a>
                                </table>
                            </form>
                            <?php if ($msg): ?>
    <p><?=$msg?></p>
